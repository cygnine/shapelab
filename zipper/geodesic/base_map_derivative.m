function[v,w] = base_map_derivative(z,a,varargin)
% [v,w] = base_map_derivative(z,a,{point_id=zeros(size(z)),cut_magnitude=abs(a)^2/imag(a))
%
%     Referring to [1], evaluates the derivative of the conformal mapping
%     function f_a, which is a basic building block for the `geodesic algorithm'
...
persistent input_schema moebius d_unzip moebius_derivative
if isempty(input_schema)
  from labtools import input_schema
  from shapelab.common import moebius moebius_derivative
  from shapelab.common import normal_slit_unzip_derivative as d_unzip
end
...
H = [factor 0; -1/b 1];
w = moebius(z, H);
dw = moeibus_derivative(z, H);

%[v,w] = unzip_at_c(w,opt.cut_magnitude,'point_id', opt.point_id);
[v,w] = d_unzip(w,opt.cut_magnitude,'point_id',opt.point_id);
