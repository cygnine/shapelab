...
%     vertices_ext) pairs. These are assumed to come from a 2*pi-periodic
...
persistent force_inverse_terminal_map zipup unzip terminal_map
...
% Send: vertices(end)   ---> 0
%       vertices(1)     ---> Inf
%       vertices(end-1) ---> \pm 1 (no good reason -- need to close map)
...
H_int = find_moebius([vertices_int(1) vertices_int(end) -1], ...
                     [Inf,            0,                -1]);

H_ext = find_moebius([vertices_ext(1) vertices_ext(end) -1], ...
                     [Inf,            0,                -1]);
...
% The points in the last bin are just zipped up now in limbo -- leave 'em.
if map_to_exterior
  slit_interior(bin_indices_int(end,1):bin_indices_int(end,2)) = false;
end
if map_to_interior
  slit_exterior(bin_indices_ext(end,1):bin_indices_ext(end,2)) = false;
end
...
% The last bin must be 'symmetrized' and 'negativized'. To do this, send
% vertices(1) to infinity and fix i and 0.
...
%    asdf
  ...
  z(interior) = unzip(i*assumptions.tooth_length, z(interior), [], []);
  ...
  %if any(diff(z(interior))>0)
  %  asdf
  %end
end

% Apply terminal map to everyone
z = terminal_map(z, assumptions);
...
psi_out = reshape(psi_out + fprint_ext_bias, psi_out_size);
psi_in = reshape(psi_in + fprint_int_bias, psi_in_size);
