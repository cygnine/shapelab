...
for q = assumptions.N_teeth:-1:1
  zipup_int = true;
  zipup_ext = true;
  % First make sure that the vertices are distinct
  tol = 1e-14;
  if imag(vertices_int(q+1))~=0 | abs(vertices_int(q+1))<tol
    zipup_int = false;
  end
  if imag(vertices_ext(q+1))~=0 | abs(vertices_ext(q+1))<tol
    zipup_ext = false;
  end
  bin_id = q + 1;
  ...
  z1 = vertices_ext(q+1);
  z2 = vertices_int(q+1);
  if zipup_int & zipup_ext
    M(q,:) = (inv([z1*rho, rho; -z2*rho, -rho])*[z1; z2]).'; % Faster than calling specify_points
  elseif not(zipup_int) & not(zipup_ext)
    M(q,:) = [0 1];
  elseif not(zipup_int)
    M(q,:) = [0 z1/0.1];
  else
    M(q,:) = [0 z2/0.1];
  end
  ...
  % remaining vertices
  if not(zipup_int & zipup_ext)
    error('What is to be done when vertices coincide?')
  end
